...
requirejs(['jquery', 'd3'],
  function ($, d3) {
    ...
    const BOXHEIGHT = 26;
    const WIDTH = 3000;
    const HEIGHT = attention.right_text.length * BOXHEIGHT * 2 + 100 + 700;
    ...
    function renderHorizLines(svg, id, start_pos, end_pos) {
      var attnMatrix = config.attention.attn[config.layer][config.att_head];
    ...
    function showComputation(svg, query_index) {
      ...
      var att = att_dets.attn[config.layer][config.att_head][query_index];
    ...
    function visualize() {
      var num_heads = attention['queries'][0].length; // Num heads for layer 0 (same as for all layers)
      config.vector_size = attention['queries'][0][0][0].length; // Layer 0, head 0, position 0 length
    ...
    function render() {
      ...
      var keys = att_dets.keys[config.layer][config.att_head];
      var att = att_dets.attn[config.layer][config.att_head];
